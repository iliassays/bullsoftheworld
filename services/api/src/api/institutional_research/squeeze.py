...
LIMITATIONS = [
    ...
    f"States are a broad diagnostic taxonomy ({METHODOLOGY_VERSION}), not a trade list. "
    "A separate selective strategy may consume point-in-time confirmations only after its "
    "registered evidence gates pass; nothing here is a prediction or recommendation.",
...
def _build_entry(
    ...
    *,
    ...
    return SqueezeEntryOut(
        ...
        paper_book_status=(
            (
                "Research inventory only: this archive row never creates a target. A separate "
