"""Run the preregistered Keltner-channel momentum diagnostic on production data.

Read-only by construction. The command writes one local JSON artifact and never creates an Atlas
strategy, Agent Decision, paper target, or UI state.

Usage:

    uv run python scripts/keltner_channel_research.py \
        --market DSE --output /tmp/dse-keltner-momentum-v1.json
"""
...
CHUNK_SIZE = 200
PRIMARY_NAME = "primary_ema20_atr20_2x"
NEIGHBOR_ATR10 = "neighbor_ema20_atr10_2x"
NEIGHBOR_WIDE = "neighbor_ema20_atr20_2_5x"


def _specs(market: str) -> dict[str, KeltnerSpec]:
    is_dse = market == "DSE"
    common = {
    ...
    prefix = "dse" if is_dse else "us"
    return {
        PRIMARY_NAME: KeltnerSpec(
            key=f"{prefix}_keltner_momentum_v1",
            ema_period=20,
            atr_period=20,
            atr_multiple=2.0,
            **common,
        ),
        NEIGHBOR_ATR10: KeltnerSpec(
            key=f"{prefix}_keltner_momentum_v1_atr10_robustness",
            ema_period=20,
            atr_period=10,
            atr_multiple=2.0,
            **common,
        ),
        NEIGHBOR_WIDE: KeltnerSpec(
            key=f"{prefix}_keltner_momentum_v1_2_5x_robustness",
            ema_period=20,
            atr_period=20,
            atr_multiple=2.5,
            **common,
...
def _directions(market: str) -> tuple[Direction, ...]:
    return ("long",) if market == "DSE" else ("long", "short")
                ...
                for direction in _directions(market):
                    result[name][direction].extend(
                        scan_keltner_trades(
                            ...
                            stressed_one_way_cost=stress_cost,
                        )
                    )
...
def _summary(
    ...
    *,
    ...
    observations: list[tuple[KeltnerTrade, float, float]] = []
    benchmark_sign = 1.0 if direction == "long" else -1.0
    for trade in trades:
        ...
        if market_return is not None:
            observations.append(
                (
                    trade,
                    market_return,
                    trade.normal_return - benchmark_sign * market_return,
                )
            )
    if not observations:
        return {
    ...
    gross = [row[0].gross_return for row in observations]
    ...
    ci_low, ci_high = _circular_block_bootstrap(
        cohort_excess,
        block=63,
    )
    ...
    return {
        ...
        "mean_gross_pct": _rounded(statistics.fmean(gross) * 100),
        ...
        "mean_benchmark_pct": _rounded(
            statistics.fmean(row[1] for row in observations) * benchmark_sign * 100
        ),
        ...
        "exit_reasons": {
            ...
            for reason in ("middle_cross", "timeout")
        },
    }
...
def _payload(
    *,
    ...
    for name, direction_rows in trades.items():
        ...
        for direction, rows in direction_rows.items():
            results[name][direction] = {
                window: _summary(
                    [trade for trade in rows if _window(market, trade.signal_date) == window],
                    market=market,
                    direction=direction,
                    benchmark=benchmark,
                )
    ...
    primary_long = results[PRIMARY_NAME]["long"]
    gate_results = {window: _gates(primary_long[window]) for window in ("validation", "holdout")}
    base_quantitative_pass = all(all(window.values()) for window in gate_results.values())
    neighbor_signs = {
        name: {
            window: (results[name]["long"][window].get("cohort_mean_stressed_pct") or 0) > 0
            for window in ("validation", "holdout")
        }
        for name in (NEIGHBOR_ATR10, NEIGHBOR_WIDE)
    }
    stability_pass = all(
        passed for windows in neighbor_signs.values() for passed in windows.values()
    )
    return {
        "registered_document": "docs/research/keltner-channel-preregistration-2026-07-26.md",
        "market": market,
        "universe": {
            "codes": len(codes),
            "first_benchmark_date": min(benchmark) if benchmark else None,
            "last_benchmark_date": max(benchmark) if benchmark else None,
            "survivorship": (
                "current active product universe; positive results are an upper bound"
            ),
            "price_basis": (
        ...
        "results": results,
        "gates": {
            "primary_long": gate_results,
            "neighbor_stressed_signs": neighbor_signs,
            "base_quantitative_pass": base_quantitative_pass,
            "parameter_stability_pass": stability_pass,
            "eligible_for_portfolio_diagnostic": (base_quantitative_pass and stability_pass),
        },
        "atlas_action": {
            ...
            "reason": (
                "This is an isolated historical diagnostic. Portfolio admission additionally "
                "requires a registered concurrency/capacity simulation and certified data."
            ),
        },
    }
...
async def async_main(market: str, output: Path) -> None:
    try:
        ...
        for direction, windows in payload["results"][PRIMARY_NAME].items():
            print(f"\n{market} primary {direction}")
            for window, result in windows.items():
                print(
                    f"  {window:10s} n={result.get('trades', 0):>5} "
                    f"net={result.get('mean_net_pct')}% "
                    f"median={result.get('median_net_pct')}% "
