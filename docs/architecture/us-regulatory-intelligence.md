...
- Shared issuers and multiple share classes retain one filing projection per ticker. Per-share
  ...
  can be established.
- A 13F CUSIP is accepted only through an existing verified mapping or exact normalized issuer and
  share-class match. Ambiguous and unresolved CUSIPs are counted and excluded, not guessed.
...
3. Run the current and prior 13F archives. Sample new, increased, reduced, exited, and unresolved
   positions against linked SEC filings.
